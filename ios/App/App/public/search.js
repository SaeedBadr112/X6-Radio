...
if (countryObj) {
    const variants = getAllCountryNameVariants(countryObj);
    matchCountry = variants.some(v => v.includes(lowerKeyword));
}
return st.name.toLowerCase().includes(lowerKeyword) || matchCountry || (st.genre || '').toLowerCase().includes(lowerKeyword);
    });
    ...
    if (isMobile()) {
        showMobileSearchResults(uniqueResults, keyword);
    } else {
...
function renderSearchResults(results) {
    ...
    container.innerHTML = results.map(st => {
        const countryObj = allCountries.find(c=>c.code===st.countryCode);
        const cName = countryObj ? getCountryDisplayName(countryObj, currentLanguage) : (st.countryCode === "XX" ? (currentLanguage === 'en' ? "Worldwide" : "عالمي") : st.countryCode);
        const isApiStation = st.isFromAPI || false;
        const isPlaying = currentStation && currentStation.id === st.id && !audioPlayer.paused;
        
        if (isMobileView) {
            return `
                <div class="station-card" data-id="${st.id}">
                    <div class="station-card-left">
                        <button class="play-station-btn search-play ${isPlaying ? 'playing' : ''}" 
                                data-id="${st.id}" 
                                data-url="${st.streamUrl}" 
                                data-name="${escapeHtml(st.name)}" 
                                data-country="${cName}" 
                                data-iswebpage="${st.isWebPage || false}" 
                                data-isapi="${isApiStation}" 
                                title="${t('play')}">
                            <i class="fas ${isPlaying ? 'fa-stop' : 'fa-play'}"></i>
                        </button>
                    </div>
                    <div class="station-card-info">
                        <span class="station-name">${isApiStation ? '🌐 ' : ''}${escapeHtml(st.name)}${isApiStation ? '<small style="color:#6a1b9a;"> (من Radio Browser)</small>' : ''}</span>
                    </div>
            ...
            return `<div class="station-card" data-id="${st.id}">
                ...
                <button class="fav-star ${isFavorite(st.id) ? 'active-fav' : ''}" data-id="${st.id}">
    ${isFavorite(st.id) 
        ? '<i class="fa-solid fa-heart" style="color: #ff1744;"></i>' 
        : '<i class="fa-regular fa-heart" style="color: #ff1744;"></i>'}
</button>
                <button class="repair-station-btn" data-id="${st.id}" title="${t('repair_url')}"><i class="fas fa-wrench"></i></button>
...
async function searchRadioBrowser(query) {
    ...
    setStatus(t('search_server_unavailable'), true);
}
        return [];
    }
}