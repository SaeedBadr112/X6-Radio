(async function firstRunLanguageSelection() {
    if (window.electronAPI && window.electronAPI.getConfigLanguage) {
        try {
            ...
            const supportedLanguages = ['ar', 'en', 'es', 'fr', 'de', 'pt'];
            if (!lang || !supportedLanguages.includes(lang)) {
                ...
                // ===== نافذة اختيار اللغة بكل اللغات الست =====
                const languages = [
                ...
                dialog.innerHTML = `
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 12px; justify-content: center; margin-bottom: 10px;">
                        ${languages.map(lang => `
                            <button id="lang${lang.code}Btn" style="background: #2563eb; border: none; color: white; padding: 10px 22px; border-radius: 40px; font-size: 16px; font-weight: bold; cursor: pointer; transition: 0.2s; flex: 0 1 auto; min-width: 80px;">
                                ${lang.label}
                            </button>
                        `).join('')}
                    </div>
                ...
                languages.forEach(lang => {
                    const btn = dialog.querySelector(`#lang${lang.code}Btn`);
                    if (btn) {
                ...
                // لا نفعل شيئاً عند النقر على الخلفية
                overlay.addEventListener('click', (e) => {
                    if (e.target === overlay) { /* المستخدم يجب أن يختار لغة */ }
                });
...
function updateCurrentStationNameAndCountry() {
    ...
    if (currentStation && currentStation.name) {
        ...
        if (currentStation.countryCode) {
            ...
            if (countryObj) {
                // استخدام الدالة العامة إذا كانت متوفرة
                if (typeof window.getCountryDisplayName === 'function') {
                    countryDisplay = window.getCountryDisplayName(countryObj, currentLanguage);
                } else {
                    // fallback للغات المدعومة
                    const langMap = {
                        'en': countryObj.name,
                        'ar': countryObj.nameAr,
                        'es': countryObj.nameEs,
                        'fr': countryObj.nameFr,
                        'de': countryObj.nameDe,
                        'pt': countryObj.namePt
                    };
                    countryDisplay = langMap[currentLanguage] || countryObj.name;
                }
            }
        }
