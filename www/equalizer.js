...
slider.addEventListener('input', function() {
    ...
    // تحديث خلفية الشريط فوراً (حتى لو لم تكن المرشحات جاهزة)
    ...
    if (typeof window.updateEqualizer === 'function') {
        if (window.audioFilters && window.audioFilters.length === EQ_BANDS) {
            window.updateEqualizer(currentEqGains);
        } else {
            console.warn('⚠️ Equalizer not ready, retrying...');
...
// ===== دالة تحديث خلفية شريط المعادل =====
function updateSliderBackground(slider) {
    const val = (slider.value - slider.min) / (slider.max - slider.min);
    const percent = val * 100;
    // الحصول على لون الثيم الحالي من المتغير العام (يُحدد في CSS)
    // نأخذ اللون من الجذر أو نستخدم لون افتراضي
    let accentColor = getComputedStyle(document.body).getPropertyValue('--accent').trim();
    if (!accentColor || accentColor === '') {
        accentColor = '#3d9dff'; // لون افتراضي
    }
    // التدرج من الأسفل إلى الأعلى (لأن الأشرطة عمودية)
    slider.style.background = `linear-gradient(to top, ${accentColor} 0%, ${accentColor} ${percent}%, #ffffff ${percent}%, #ffffff 100%)`;
}
// ===== تحديث خلفيات جميع الأشرطة عند تغيير الثيم =====
function updateAllSliderBackgrounds() {
    document.querySelectorAll('.eq-slider').forEach(slider => {
        updateSliderBackground(slider);
    });
}
window.updateAllSliderBackgrounds = updateAllSliderBackgrounds; // تصديرها للاستخدام الخارجي
